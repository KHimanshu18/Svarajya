...
function AuthGatewayContent() {
  ...
  useEffect(() => {
    if (errorParam) {
      ...
      if (errorParam === "Email_Registered") {
        setError(
          "This email is already registered with email and password. Please login using email and password instead.",
        );
      } else {
  ...
  useEffect(() => {
    if (successParam) {
      ...
      if (successParam === "Google_Signup") {
        setMsg(
          "Account created successfully with Google. Welcome to Svarajya!",
        );
  ...
  useEffect(() => {
    ...
    if (
      isGoogleAuthRef.current ||
      successParam === "Google_Signup" ||
      successParam === "Google_Login"
    ) {
      return;
    }
  ...
  // ------------------------------------------
  const refreshAttemptState = useCallback(
    (currentEmail: string, storageKey: string) => {
  ...
  useEffect(() => {
    const key =
      mode === "login" ? LOCKOUT_STORAGE_KEY_LOGIN : LOCKOUT_STORAGE_KEY_SIGNUP;
  ...
  const checkRateLimit = (storageKey: string): boolean => {
    ...
    if (lockSecs > 0) {
      ...
      setAttemptsLeft(0);
      setError(
        `Too many attempts. Please try again in ${Math.ceil(lockSecs / 60)} minute(s).`,
      );
    ...
    try {
      if (mode === "signup") {
        ...
        if (responseData.user) {
          try {
            ...
            if (!profileRes.ok) {
              ...
              console.log(
                "User successfully created in Prisma via public create-user endpoint",
              );
            }
        ...
        const checkRes = await fetch(
          `/api/check-user?email=${encodeURIComponent(trimmedEmail)}`,
        );
        ...
        if (signInError) {
          ...
          if (left <= 0) {
            ...
            setRateLimitSeconds(lockSecs);
            setError(
              `Too many failed attempts. Try again in ${Math.ceil(lockSecs / 60)} minute(s).`,
            );
            ...
            try {
              ...
              if (exists) {
                if (provider === "GOOGLE") {
                  setError(
                    "This account was registered with Google. Please use 'Continue with Google' to login.",
                  );
                } else {
            ...
            setError(
              <span>
                Email not confirmed. Please check your inbox and click the
          ...
          {mode === "registration_success" && (
            ...
            >
              ...
              <p className="text-sm text-white/70 leading-relaxed px-4">
                Registration is completed. Please check your email ID for
                account opening.
              </p>
              ...
              <div className="pt-4 w-full">
                ...
                <p className="text-[10px] text-white/50 mt-4 tracking-wider">
                  Once you confirm your email, you can log in using the button
                  below.
                </p>
              </div>
            </motion.div>
          ...
          {(mode === "login" ||
            ...
            >
                  ...
                  {mode === "signup" && (
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[10px] text-amber-500/70 italic px-2">
                        Use the name exactly as it appears on your Aadhaar card
                        — no nicknames.
                      </p>
                    </div>
                  )}
                      ...
                      <div className="space-y-2 px-1">
                        {[
                          ...
                          >
                            ...
                            <span
                              className={
                                r.ok ? "text-emerald-400/90" : "text-white/40"
                              }
                            >
                              {r.label}
                            </span>
              ...
              >
                ...
                >
                  <path
                    d="M22.56 12.25C22.56 11.47 22.49 10.72 22.36 10H12V14.26H17.92C17.67 15.63 16.86 16.79 15.69 17.57V20.34H19.26C21.35 18.41 22.56 15.58 22.56 12.25Z"
                    fill="#4285F4"
                  />
                  <path
                    d="M12 23C14.97 23 17.46 22.01 19.26 20.34L15.69 17.57C14.71 18.23 13.46 18.66 12 18.66C9.18 18.66 6.78 16.75 5.91 14.18H2.21V17.05C4.01 20.64 7.72 23 12 23Z"
                    fill="#34A853"
                  />
                  <path
                    d="M5.91 14.18C5.69 13.51 5.56 12.78 5.56 12C5.56 11.22 5.69 10.49 5.91 9.82V6.95H2.21C1.47 8.43 1.04 10.16 1.04 12C1.04 13.84 1.47 15.57 2.21 17.05L5.91 14.18Z"
                    fill="#FBBC05"
                  />
                  <path
                    d="M12 5.34C13.62 5.34 15.07 5.9 16.21 6.99L19.34 3.86C17.46 2.1 14.97 1 12 1C7.72 1 4.01 3.36 2.21 6.95L5.91 9.82C6.78 7.25 9.18 5.34 12 5.34Z"
                    fill="#EA4335"
                  />
                </svg>
              ...
              : <button
                ...
                >
                  {mode === "signup" ?
                    "Already have a Rajya? Login instead"
                  : "Need to establish your Rajya? Sign up"}
                </button>
              }
            </motion.div>
          )}
        ...
        <div className="flex items-center justify-center gap-5">
          {TRUST_ICONS.map((t, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              ...
              <span className="text-[9px] text-[#f2faf5] text-center leading-tight ">
                {t.label}
              </span>
            </div>
          ))}
