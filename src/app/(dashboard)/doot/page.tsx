          ...
          : notifications.map((n) => {
              ...
              return (
                <button
                  key={n.id}
                  onClick={() => handleTap(n)}
                  className={`w-full text-left p-4 rounded-xl border transition-all ${style.bg} ${style.border} ${
                    !n.read ? "opacity-100" : "opacity-50"
                  } hover:opacity-100`}
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${style.dot} mt-1.5 shrink-0 ${!n.read ? "animate-pulse" : ""}`}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p
                          className={`text-sm font-medium ${!n.read ? "text-white" : "text-white/60"}`}
                        >
                          {n.title}
                        </p>
