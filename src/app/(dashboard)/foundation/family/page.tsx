...
import {
  FamilyTreeGame,
  FamilyMember,
} from "@/components/module1/FamilyTreeGame";
...
export default function Submodule1B() {
        ...
        if (Array.isArray(profileData)) {
          const loadedMembers = profileData.map((member: any) => ({
            ...
            accessRole:
              ...
              : member.accessLevel === "emergency" ? "Emergency-only"
              : "None",
          }));
  ...
  const handleAddMember = async (memberData: Omit<FamilyMember, "id">) => {
    ...
    try {
      const payload = {
        ...
        accessLevel:
          memberData.accessRole === "Executor" ? "write"
          : memberData.accessRole === "Viewer" ? "read"
          : memberData.accessRole === "Emergency-only" ? "emergency"
          : "read",
  ...
  const loadIndex =
    members.length > 0 ?
      Math.round((dependencyCount / members.length) * 100)
    : 0;
          ...
          <div>
            ...
            {members.length > 0 && (
              <p className="text-xs text-amber-400 mt-1">
                {members.length}/5 members in sync
              </p>
            )}
          </div>
        </div>
            ...
            >
              <div className="space-y-4">
                ...
                <p className="text-sm text-white/55 leading-relaxed">
                  Indian financial planning is deeply family-linked. Knowing who
                  depends on you helps us prioritise your insurance and
                  nomination coverage correctly.
                </p>
              </div>

              <VideoTutorialPlaceholder
                youtubeId="hU0V-FwTmWk"
                label="Why family planning matters for financial protection"
              />
            ...
            >
              <div className="text-center space-y-3">
                ...
                <p className="text-white/40 text-sm">
                  {members.length} members verified in database
                </p>
              </div>

              <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
                <h3 className="text-xs uppercase tracking-widest text-amber-400 mb-3">
                  Financial Dependency Summary
                </h3>
                <div className="flex items-end justify-center gap-2 mb-4">
                  <span className="text-5xl font-bold text-white">
                    {dependencyCount}
                  </span>
                  <span className="text-base text-white/50 mb-1">
                    dependants identified
                  </span>
                </div>
                  ...
                  <div className="text-left bg-red-500/8 border border-red-500/20 p-4 rounded-xl flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/50">
                        {loadIndex}% of your family are financially dependent on
                        you. We&apos;ll prioritise life and health insurance
                        coverage in your next steps.
                      </p>
                    </div>
                  </div>
                : <p className="text-sm text-white/50 px-4 py-2">
                    No financial dependants. Your planning focus will be
                    personal growth and investments.
                  </p>
                }
              </div>
