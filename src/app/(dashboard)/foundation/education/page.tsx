...
const DEGREE_OPTIONS = [
  ...
  "12th Standard",
  "Diploma",
  "Bachelor's (B.A / B.Sc / B.Com)",
  "Bachelor's (B.Tech / B.E.)",
  "Bachelor's (BBA / BCA)",
  "Master's (M.A / M.Sc / M.Com)",
  "Master's (M.Tech / MBA)",
  "PhD / Doctorate",
  "Professional (CA / CS / CMA)",
  "Professional (MBBS / MD)",
  "Professional (LLB / LLM)",
  "Other",
];
...
export default function EducationPage() {
            ...
            const loadedEntries = educationData.map((edu: any) => {
              ...
              if (edu.familyMemberId && familyData.length > 0) {
                const member = familyData.find(
                  (fm: any) => fm.id === edu.familyMemberId,
                );
    ...
    if (year) {
      ...
      if (isNaN(certYear) || certYear < 1900) {
        ...
        if (personDob) {
          ...
          if (certYear < minYear) {
            setYearError(
              `Passing year cannot be before ${minYear} (birth year + 15)`,
            );
          }
        }
      }
  ...
  const handleAddEntry = async () => {
    ...
    if (year) {
      ...
      if (personDob) {
        ...
        if (certYear < minYear) {
          ...
          toast(
            `Passing year must be at least ${minYear} (15 years after birth)`,
            "error",
          );
          return;
        }
    ...
    try {
      ...
      setEntries((prev) =>
        prev.map((e) => (e.id === tempId ? { ...e, id: savedRecord.id } : e)),
      );
  ...
  const handleViewCert = async (certId: string) => {
    if (
      !certId.startsWith("http") &&
      certId.length > 20 &&
      !certId.startsWith("opfs")
    ) {
      ...
      try {
        ...
        if (data.success && data.data.webViewLink) {
          ...
          toast(
            data.error ||
              "Failed to fetch from Google Drive. Please try relogging with Google.",
            "error",
          );
          return;
        }
    ...
    <div className="flex flex-col min-h-screen relative p-6 pb-24">
      <div className="flex items-center gap-3 pt-8 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-white/35 mt-0.5">
            Add your degrees, diplomas & certificates
          </p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <VideoTutorialPlaceholder
          youtubeId="KNWL0uda_OA"
          label="Why education matters for your financial profile"
        />
      </div>
      ...
      <div className="bg-[var(--color-rajya-accent)]/8 border border-[var(--color-rajya-accent)]/20 rounded-xl p-3 mb-5">
        <p className="text-xs text-[var(--color-rajya-muted)]">
          💡 Your education determines earning potential and links to any
          student loans. Add each degree separately — you can upload
          certificates too.
        </p>
      </div>
      ...
      {entries.length > 0 && (
        <div className="space-y-3 mb-5">
          <p className="text-[10px] text-white/30 uppercase tracking-wider">
            Your Qualifications ({entries.length})
          </p>
          {entries.map((e, i) => (
            <div
              key={e.id || i}
            ...
            >
              <div className="absolute top-3 right-3 flex items-center gap-2">
                <button
                  onClick={() =>
                    router.push(`/foundation/education/${e.id}/edit`)
                  }
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-sm font-medium text-[var(--color-rajya-text)] pr-6">
                  {e.degree}
                </p>
              </div>
              <p className="text-xs text-[var(--color-rajya-muted)]">
              ...
              {e.specialization && (
                <p className="text-[10px] text-[var(--color-rajya-muted)]/60 mt-0.5">
                  {e.specialization}
                </p>
              )}
              {e.certificateId && (
          ...
          >
            ...
            <div>
              ...
              {familyMembers.length === 0 && (
                <p className="text-[10px] text-white/25 mt-1">
                  Add family members in Foundation → Family to select them here
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                />
                {yearError && (
                  <p className="text-[10px] text-red-400 mt-1">{yearError}</p>
                )}
              </div>
              <div>
                <label className="text-xs text-[var(--color-rajya-muted)] mb-1 block">
            ...
            <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-3">
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full bg-white transition-transform ${hasLoan ? "translate-x-5" : "translate-x-0"}`}
                />
              </button>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-8">
              ...
              <p className="text-[10px] text-[var(--color-rajya-muted)] mb-4">
                Upload your degree/diploma certificate. Saved securely in your
              ...
              {isGoogleLinked === false ?
                <div className="p-6 border-2 border-dashed border-blue-500/20 rounded-2xl bg-blue-500/5 text-center">
                  ...
                  <p className="text-sm text-white font-medium">
                    Link Google Drive First
                  </p>
                  <p className="text-xs text-white/40 mt-1 mb-4">
                    Education certificates are stored securely in your own
                    Google Drive.
                  </p>
                  <button
                    onClick={() =>
              ...
              : <FileUploader
                  ...
                  onUploaded={(url) =>
                    setUploadedCerts((prev) => ({ ...prev, new: url || "" }))
                  }
      ...
      {anyLoan && (
        <div className="bg-[var(--color-rajya-danger)]/10 border border-[var(--color-rajya-danger)]/30 rounded-xl p-4 flex items-start gap-3 mb-5">
          ...
          <div>
            <p className="text-sm font-bold text-[var(--color-rajya-danger)] mb-1">
              Education Loan Detected
            </p>
            <p className="text-xs text-[var(--color-rajya-danger)]/80">
              This will be tracked in your Rin (Debt) module. Make sure to
              record EMI details there.
            </p>
          </div>
        </div>
      )}
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-white/40 text-center mb-6">
                This action cannot be undone. This qualification will be removed
                from your profile.
              </p>
                ...
                <button
                  onClick={() => {
                    const entryIndex = entries.findIndex(
                      (_, idx) =>
                        `local-${idx}` === isDeleting ||
                        entries[idx].id === isDeleting,
                    );
