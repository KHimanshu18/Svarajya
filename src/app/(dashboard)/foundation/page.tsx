...
const STEPS = [
  {
    ...
    desc: "Who depends on you financially?",
    route: "/foundation/family",
  },
  {
    id: "education",
    icon: <GraduationCap className="w-5 h-5" />,
    label: "Education & Qualifications",
    desc: "Your background and any education loans",
    route: "/foundation/education",
  },
];
          ...
          OnboardingStore.set(
            {
              ...
              occupationType:
                profileData.occupationType || data.occupationType || "",
              email: profileData.email || data.email || "",
              mobile: profileData.phone || data.mobile || "",
              familyMembers:
                profileData.familyMembers || data.familyMembers || [],
            },
            { sync: false },
          );
        }
    ...
    return (
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            >
              <p className="font-semibold text-white truncate">
                {profile?.name || data.fullName || "Your Name"}
              </p>
              <p className="text-xs text-white/40 mt-0.5">
                {profile?.occupationType || data.occupationType || "Occupation"}{" "}
                · {getLifePhase(profile?.dob || data.dob)} phase
              </p>
                ...
                <div
                  ...
                  onClick={() =>
                    (window.location.href =
                      "/onboarding/contact-info?t=" + Date.now())
                  }
                >
                  <p className="text-xs text-white/30 mt-0.5">
                    +91 {profile?.phone || data.mobile}
                  </p>
                  {profile?.isMobileVerified && (
                    <span className="text-emerald-400 text-[10px] uppercase tracking-[0.15em] font-medium">
                      Verified
                    </span>
                  )}
                </div>
          ...
          {!photoUploaded && !uploadError && (
            <p className="text-xs text-white/25 mt-3 text-center">
              Tap the circle above to add your photo (Max 2MB)
            </p>
          )}
        </div>
        ...
        <div className="mt-4">
          ...
          <VideoTutorialPlaceholder
            ...
            label="How to build a strong financial foundation"
          />
        </div>
