...
const parseJsonResponse = async (response: Response) => {
  ...
  throw new Error(
    `Unexpected response format (${response.status}): ${text.slice(0, 200)}`,
  );
};
...
export default function BhoomiPage() {
  ...
  const loadProperties = async () => {
    ...
    try {
      const response = await fetch("/api/bhoomi/properties", {
        cache: "no-store",
      });
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error?.message ||
            `Unable to load properties (${response.status})`,
        );
      }
  ...
  const handleDeleteConfirmed = async (propertyId: string) => {
    ...
    try {
      const response = await fetch(`/api/bhoomi/properties/${propertyId}`, {
        method: "DELETE",
      });
      ...
      NotificationStore.push({
        ...
        message: "The property was removed successfully.",
        link: "/bhoomi",
      });
    } catch (err) {
      NotificationStore.push({
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => router.push("/bhoomi/add")}
            ...
            <div className="space-y-4 pr-2">
              ...
              : <div className="grid gap-4">
                  {properties.map((property, idx) => (
                    <div
                      key={property.id}
                      className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-xl"
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <button
                            onClick={() =>
                              router.push(`/bhoomi/${property.id}/edit`)
                            }
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl bg-white/5 p-4">
                          ...
                          <p className="mt-2 text-sm font-semibold text-[var(--color-rajya-text)]">
                        ...
                        <div className="rounded-2xl bg-white/5 p-4">
                          ...
                          {property.marketValue && property.marketValue > 0 ?
                            (() => {
                              const rental = Number(
                                property.rentalIncomeAnnual ?? 0,
                              );
                              const costs = Number(
                                property.carryingCostsAnnual ?? 0,
                              );
                              const roi =
                                ((rental - costs) / property.marketValue) * 100;
                              const formatted = `${roi >= 0 ? "" : ""}${roi.toFixed(2)}% per year`;
                              const colorClass =
                                roi >= 0 ? "text-emerald-300" : "text-rose-400";
                              return (
                                <p
                          ...
                          : <p className="mt-2 text-sm text-[var(--color-rajya-muted)]">
                              ROI: Enter market value
                            </p>
                          }
                        </div>
                      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <ConfirmModal
                    ...
                    onConfirm={() =>
                      pendingDeleteId && handleDeleteConfirmed(pendingDeleteId)
                    }
                    onCancel={() => {
                      setConfirmOpen(false);
                      setPendingDeleteId(null);
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              <div className="mt-3">
                <VideoTutorialPlaceholder
                  ...
                  label="Property record management"
                />
              </div>
            </div>
          </aside>
