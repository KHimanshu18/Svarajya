...
export default function AddBhoomiPropertyPage() {
  ...
  const errors = useMemo(() => {
    ...
    if (carryingCostsAnnual.trim()) {
      if (Number.isNaN(carryingAmount) || carryingAmount < 0) {
        fieldErrors.carryingCostsAnnual = 'Carrying costs must be 0 or greater.';
      }
    }
    ...
    return fieldErrors;
  }, [propertyTitle, propertyType, ownershipType, marketValue, purchasePrice, purchaseDate, carryingCostsAnnual, rentalIncomeAnnual]);
  ...
  const hasRoiInputs = carryingCostsAnnual.trim() !== '' && rentalIncomeAnnual.trim() !== '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setRequestError(json?.error?.message || json?.message || 'Unable to save property');
        return;
      }
      ...
      <div className="w-full max-w-4xl bg-slate-900/95 border border-white/10 shadow-2xl shadow-slate-950/40 rounded-3xl backdrop-blur-xl overflow-hidden">
        <div className="relative bg-slate-950/80 px-6 py-5 border-b border-white/10">
          ...
          <div className="max-w-2xl mx-auto text-center pt-1 pb-3">
            ...
            <h2 className="text-3xl font-semibold text-white">Add a new property</h2>
            <p className="mt-3 text-sm text-slate-400">Capture property details, connect loans, and manage access codes securely.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 grid gap-6">
          ...
          <div className="grid gap-6">
            <label className="block">
              ...
              <input
                value={propertyTitle}
                onChange={(e) => onFieldChange(setPropertyTitle)(e.target.value)}
              ...
              />
              {submitAttempted && errors.propertyTitle && <p className="text-xs text-rose-400 mt-2">{errors.propertyTitle}</p>}
            </label>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <label className="block">
                ...
                <select
                  value={propertyType}
                  onChange={(e) => onFieldChange(setPropertyType)(e.target.value)}
                ...
                {submitAttempted && errors.propertyType && <p className="text-xs text-rose-400 mt-2">{errors.propertyType}</p>}
              </label>
              ...
              <label className="block">
                ...
                {submitAttempted && errors.ownershipType && <p className="text-xs text-rose-400 mt-2">{errors.ownershipType}</p>}
              </label>
            </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <label className="block">
                ...
                <input
                  value={marketValue}
                  onChange={(e) => onFieldChange(setMarketValue)(e.target.value)}
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  placeholder="₹0"
                />
                {submitAttempted && errors.marketValue && <p className="text-xs text-rose-400 mt-2">{errors.marketValue}</p>}
              </label>
              <label className="block">
                <span className="text-sm text-slate-400 mb-2 block">Purchase price</span>
                <input
                  value={purchasePrice}
                  onChange={(e) => onFieldChange(setPurchasePrice)(e.target.value)}
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  placeholder="₹0"
                />
                {submitAttempted && errors.purchasePrice && <p className="text-xs text-rose-400 mt-2">{errors.purchasePrice}</p>}
              </label>
            </div>
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              <label className="block">
                ...
                <input
                  type="date"
                  value={purchaseDate}
                  onChange={(e) => onFieldChange(setPurchaseDate)(e.target.value)}
                ...
                />
                {submitAttempted && errors.purchaseDate && <p className="text-xs text-rose-400 mt-2">{errors.purchaseDate}</p>}
              </label>
              <label className="block">
                <span className="text-sm text-slate-400 mb-2 block">Link loan (optional)</span>
                <select
                  value={loanId ?? ''}
                  onChange={(e) => onFieldChange(setLoanId)(e.target.value || null)}
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <label className="block">
                <span className="text-sm text-slate-400 mb-2 block">Annual carrying costs (₹)</span>
                <input
                  value={carryingCostsAnnual}
                  onChange={(e) => onFieldChange(setCarryingCostsAnnual)(e.target.value)}
                  type="number"
                  min="0"
                  step="0.01"
                ...
                />
                {submitAttempted && errors.carryingCostsAnnual && <p className="text-xs text-rose-400 mt-2">{errors.carryingCostsAnnual}</p>}
              </label>
              <label className="block">
                <span className="text-sm text-slate-400 mb-2 block">Annual rental income (₹)</span>
                <input
                  value={rentalIncomeAnnual}
                  onChange={(e) => onFieldChange(setRentalIncomeAnnual)(e.target.value)}
                  type="number"
                  min="0"
                  step="0.01"
                ...
                />
                {submitAttempted && errors.rentalIncomeAnnual && <p className="text-xs text-rose-400 mt-2">{errors.rentalIncomeAnnual}</p>}
              </label>
            </div>
            ...
            <label className="block">
              <span className="text-sm text-slate-400 mb-2 block">Secret field</span>
              <div className="relative">
                <input
                  type={showSecretValue ? 'text' : 'password'}
                  value={secretFieldValue}
                  onChange={(e) => onFieldChange(setSecretFieldValue)(e.target.value)}
                ...
                >
                  {showSecretValue ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </button>
              </div>
              <p className="text-xs text-slate-500 mt-2">Use this to store access codes or private property credentials.</p>
            </label>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-slate-400">Complete the form and save the property to continue.</div>
            <div className="flex gap-3 flex-wrap">
              ...
              >
                {saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : 'Save property'}
              </button>
              <button
                type="button"
