          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/60">
                Manage your income tax return filings.
              </p>
            </div>
          </div>
          <div>
              ...
              >
                <div>
                  <div className="text-sm text-white/60">
                    {r.assessmentYear} · {r.filingType || "ITR type not set"}
                  </div>
                  <div className="mt-1 text-lg font-semibold text-white">
                    {r.status || "Unknown"}
                  </div>
                  <div className="mt-2 text-sm text-white/60">
                    Paid: ₹{r.taxPaid ?? 0} · Payable: ₹{r.taxDue ?? 0}
                  </div>
                  {r.documentUrl || (r as any).documentId ?
                    <a
                      className="mt-2 inline-block text-sky-300 underline"
                      href={getFileUrl(r.documentUrl || (r as any).documentId)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      View Document
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => router.push(`/kar/itr/${r.id}/edit`)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6">
          <div className="w-full max-w-sm bg-slate-900/95 rounded-2xl p-6">
            ...
            <p className="text-sm text-white/70 mb-6">
              Are you sure you want to delete this ITR record? This action
              cannot be undone.
            </p>
