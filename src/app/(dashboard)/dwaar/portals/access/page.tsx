...
export default function AccessOverviewPage() {
  ...
  const readinessPercent =
    portals.length > 0 ? Math.round((readyCount / portals.length) * 100) : 0;
  const readinessLabel =
    readinessPercent >= 80 ? "High"
    : readinessPercent >= 40 ? "Medium"
    : "Low";
  const readinessColor =
    readinessPercent >= 80 ? "text-emerald-400"
    ...
    <div className="flex flex-col min-h-screen p-6 pb-24 relative">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <div className="flex items-center gap-3 pt-8 mb-6">
          <button
            onClick={() => router.push("/dwaar/portals")}
          ...
          <div>
            ...
            <p className="text-xs text-white/35 mt-0.5">
              Executor assignments and emergency readiness.
            </p>
          </div>
        </div>
          ...
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden mt-2">
            <div
              className={`h-full rounded-full transition-all duration-700 ${
                readinessPercent >= 80 ? "bg-emerald-400"
                : readinessPercent >= 40 ? "bg-amber-400"
                : "bg-red-400"
              }`}
        ...
        <div className="flex-1 space-y-4">
          {Object.keys(groupedByMember).length === 0 ?
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-xs text-white/20 text-center mb-5">
                Open a portal and assign an executor to improve readiness.
              </p>
            </div>
                  ...
                  <div className="bg-amber-400/5 rounded-lg px-4 py-3 border border-amber-400/10">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-semibold text-amber-400">
                        {counts.executor} Portal{counts.executor > 1 ? "s" : ""}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white/3 border border-white/8 rounded-xl p-3.5 space-y-2 mt-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <p className="text-xs text-amber-400/70">
                  {portalsWithNoExecutor.length} portal
                  {portalsWithNoExecutor.length > 1 ? "s have" : " has"} no
                  executor assigned
                </p>
              </div>
            </div>
