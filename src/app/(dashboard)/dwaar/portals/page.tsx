    ...
    <div className="flex flex-col min-h-screen p-6 pb-24 relative">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/50 mt-0.5">
                Your credential and access management vault.
              </p>
            </div>
          </div>
          <button
        ...
        <PageGuide
          ...
          actions={[
            { emoji: "🔑", label: "Add portals" },
            { emoji: "🛡️", label: "Assign access" },
            { emoji: "📊", label: "Track health" },
          ]}
        />
        <div className="h-4" />
        ...
        {portals.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {PORTAL_CATEGORIES.filter(
              (c) => (insights.byCategory[c.id] || 0) > 0,
            ).map((c) => (
              <span
                key={c.id}
                className="text-[10px] bg-white/5 border border-white/10 rounded-full px-3 py-1 text-white/60"
              >
        ...
        {unlockedMilestones.length > 0 && (
          <div className="flex gap-2 mt-3 flex-wrap">
            {unlockedMilestones.map((m) => (
              <span
                key={m.id}
                className="text-[10px] bg-amber-400/10 border border-amber-400/20 rounded-full px-2.5 py-1 text-amber-400/70"
              >
                🏆 {m.label}
              </span>
            ))}
        ...
        {portals.length > 0 && (
          <div className="mt-4 bg-white/3 border border-white/8 rounded-xl p-3.5 space-y-1.5">
            ...
            <p className="text-xs text-white/50">
              📊 {insights.totalPortals} active portal
              {insights.totalPortals > 1 ? "s" : ""} recorded
            </p>
            {insights.portalsWithNoExecutor > 0 && (
              <p className="text-xs text-amber-400/70">
                ⚠ {insights.portalsWithNoExecutor} portal
                {insights.portalsWithNoExecutor > 1 ? "s have" : " has"} no
                executor assigned
              </p>
            )}
            {insights.portalsMissingAccess > 0 && (
              <p className="text-xs text-amber-400/70">
                ⚠ {insights.portalsMissingAccess} portal
                {insights.portalsMissingAccess > 1 ? "s have" : " has"} no
                access assigned at all
              </p>
            )}
            {insights.portalsNotReviewed > 0 && (
              <p className="text-xs text-red-400/70">
                ⚠ {insights.portalsNotReviewed} portal
                {insights.portalsNotReviewed > 1 ? "s" : ""} not reviewed in
                over 1 year
              </p>
            )}
            {insights.insuranceMissingAwareness > 0 && (
              <p className="text-xs text-amber-400/70">
                🛡️ {insights.insuranceMissingAwareness} insurance portal
                {insights.insuranceMissingAwareness > 1 ? "s" : ""} lacking
                nominee awareness
              </p>
            )}
          </div>
        ...
        {portals.length > 0 &&
          (() => {
            ...
            const insurancePortals = portals.filter(
              (p) => p.category === "insurance",
            );
            const subPortals = portals.filter(
            ...
            if (taxPortals.length === 0)
              warnings.push({
                emoji: "🧾",
                message: "Compliance Gate Incomplete — no tax portals added.",
              });
            if (insurancePortals.some((p) => !p.nomineeAwareness))
              warnings.push({
                emoji: "🛡️",
                message:
                  "Insurance portal exists without nominee awareness flagged.",
              });
            if (subPortals.some((p) => !p.renewalDate))
              warnings.push({
                emoji: "📦",
                message:
                  "Subscription exists without renewal date — risk of missed charges.",
              });
            if (bankPortals.some((p) => !p.linkedFamilyMemberId))
              warnings.push({
                emoji: "🏦",
                message:
                  "Bank portal has no executor assigned for emergencies.",
              });
            ...
            return (
              <div className="mt-3 space-y-1.5">
                {warnings.map((w, i) => (
                  <div
                    key={i}
                    className="bg-amber-400/8 border border-amber-400/15 rounded-xl p-2.5 flex items-start gap-2"
                  >
          ...
          : portals.length === 0 ?
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-white/50 mb-1 font-medium">
                No Portals Added Yet
              </p>
              <p className="text-xs text-white/40 text-center mb-5">
                Start organizing your digital access for clarity and emergency
                readiness.
              </p>
              <button
                onClick={() => router.push("/dwaar/portals/add")}
        ...
        <div className="space-y-3 mt-5">
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => router.push("/dwaar/portals/access")}
        ...
        <div className="mt-4">
          <VideoTutorialPlaceholder
            youtubeId="7dbmNkrANws"
            label="Password security & digital safety essentials"
          />
        </div>
