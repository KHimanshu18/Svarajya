...
export default function ContactStep() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Recaptcha init failed");
  ...
  const handleSendOtp = async () => {
    ...
    const formattedPhone = `+91${mobileResult.normalized}`;
    ...
    <div className="flex flex-col min-h-screen p-6 relative">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        <div className="flex items-center gap-2 pt-10 mb-2">
          ...
          <p className="text-xs text-amber-400/70 uppercase tracking-widest">
            Step 5 of 5
          </p>
        </div>
        <ProgressBar step={5} />
          ...
          <div className="flex justify-center">
            <svg width="180" height="80" viewBox="0 0 180 80">
              ...
              <rect
                ...
                stroke={
                  unlocked ? "rgba(251,191,36,0.6)" : "rgba(251,191,36,0.2)"
                }
                strokeWidth="1"
          ...
          <div className="space-y-5">
            <div>
              <h1 className="text-xl font-semibold text-white mb-1">
                Secure Messenger Contact
              </h1>
              <p className="text-xs text-white/40">
                We use this only for important alerts and financial reminders.
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                {isReadOnly && (
                  <span className="text-emerald-400 text-[11px] uppercase tracking-[0.15em]">
                    Verified
                  </span>
                )}
              </div>
              <div className="flex gap-2">
                <div className="bg-white/6 border border-white/15 rounded-xl px-3 flex items-center text-white/55 text-sm shrink-0">
                  +91
                </div>
                <input
                  type="tel"
                  ...
                  autoComplete="off"
                  onChange={(e) => {
                    if (!isReadOnly && otpState !== "verified") {
            ...
            {!isReadOnly && otpState !== "verified" && (
              <>
                {!isReadOnly && (
                  <button
                    disabled={resendCountdown > 0}
                    onClick={handleSendOtp}
                    className="w-full bg-white/8 border border-white/15 text-white/70 py-3 rounded-xl text-sm hover:bg-white/12 transition-colors"
                  >
                    {resendCountdown > 0 ?
                ...
                {otpState === "sent" && (
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      className="w-full bg-amber-400 text-black font-semibold py-3 rounded-xl text-sm"
                    >
                      Verify OTP
                    </button>
                  </div>
            ...
            {otpState === "verified" && (
              <button
                ...
                className="w-full bg-amber-400 text-black font-semibold py-3 rounded-xl text-sm hover:bg-amber-300 transition-colors mt-4"
              >
                Save & Continue
              </button>
            )}
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-white/25">
                Email is synced from your login account.
              </p>
            </div>
            ...
            <div className="flex items-start justify-between bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white shadow transition-transform mx-1 ${whatsapp ? "translate-x-4" : ""}`}
                />
              </button>
            </div>
          </div>
        ...
        {otpState === "none" && (
          <div className="pb-4">
            <button
              onClick={() => router.push("/onboarding/firstwin")}
              className="w-full text-amber-400/60 text-[10px] font-semibold uppercase tracking-wider py-3 hover:text-amber-400 transition-colors"
            >
              Skip verification
            </button>
          </div>
