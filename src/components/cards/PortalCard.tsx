    ...
    return (
        ...
        >
            <div className="flex items-start gap-3">
                <span className="text-lg mt-0.5">{catMeta?.emoji || "ðŸ“Ž"}</span>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                        <p className="text-sm font-medium text-white truncate">
                            {portal.customServiceName ? `${portal.platformName} â€” ${portal.customServiceName}` : portal.platformName}
                        </p>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                        {portal.twoFAStatus === "enabled" && (
                            <span className="text-[10px] text-emerald-400/70 bg-emerald-400/10 px-2 py-0.5 rounded-full">2FA âœ“</span>
                        )}
                        {hasExecutor && (
                            <span className="text-[10px] text-amber-400/70 bg-amber-400/10 px-2 py-0.5 rounded-full">Executor Assigned</span>
                        )}
                        {emergencyReady && (
                            <span className="text-[10px] text-emerald-400/70">âœ” Emergency Ready</span>
                        )}
                        {portal.billingCycle && (
